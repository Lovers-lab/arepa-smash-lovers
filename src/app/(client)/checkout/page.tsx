...
export default function CheckoutPage() {
  ...
  const envio = deliveryZone 
    ...
    : calculateShipping(totalPostDesc)
  ...
  async function submitOrder() {
    ...
    try {
      ...
      if (metodoPago === 'TARJETA') { const mioRes = await fetch('/api/mio', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ orderId: data.orderId }) }); const mioData = await mioRes.json(); if (mioData.checkoutUrl) { router.push('/orders/' + data.orderId + '?mioUrl=' + encodeURIComponent(mioData.checkoutUrl)); return; } else { setError('Error iniciando pago: ' + (mioData.error || 'Intenta de nuevo')); setSubmitting(false); return; } }
      router.push('/')
