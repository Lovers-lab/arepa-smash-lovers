...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    // Single channel with wildcard event
    const ch = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders' }, payload => {
        ...
        if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
          new Notification(`🔔 Nuevo pedido #${(o as any).numero_pedido}`, {
            body: `${formatRD(o.total_pagado)} · ${o.metodo_pago}`,
...
COMANDA #${(order as any).numero_pedido}
${order.marca === 'AREPA' ? 'Arepa Lovers' : 'Smash Lovers'}
...
${(order as any).direccion_texto ? `Dir: ${(order as any).direccion_texto}` : ''}
${'═'.repeat(32)}
      `.trim())
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#F7F8FA', fontFamily: 'var(--font-body)' }}>
      <style>{`
        ...
        @keyframes flash { 0%,100%{background:white} 50%{background:#FEF2F2} }
        .rpl { position:relative; overflow:hidden; }
        .new-order { animation:flash 1s ease 4; border-color:#EF4444 !important; }
      `}</style>
      ...
      <header style={{ background:'white', borderBottom:'1px solid #E4E6EA', position:'sticky', top:0, zIndex:30, boxShadow:'0 1px 8px rgba(0,0,0,0.06)' }}>
        <div style={{ maxWidth:'900px', margin:'0 auto', padding:'12px 16px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
            ...
            {active.length > 0 && (
              <div style={{ background:'#EF4444', color:'white', borderRadius:'999px', padding:'3px 10px', fontSize:'12px', fontWeight:800 }}>
      ...
      <main style={{ maxWidth:'900px', margin:'0 auto', padding:'16px' }}>
        ...
        {tab === 'activos' && (
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            ...
            {active.map(order => {
              ...
              const sc = STATUS_COLORS[estado] || '#9CA3AF'
              const user = order.user as any
                ...
                <div key={order.id} className={isNew ? 'new-order' : ''}
                  ...
                  <div style={{ background:'#F7F8FA', padding:'10px 16px', borderBottom:'1px solid #F0F2F5', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'8px' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'8px', flexWrap:'wrap' }}>
                      <span style={{ fontFamily:'var(--font-display)', fontWeight:800, fontSize:'18px' }}>#{(order as any).numero_pedido}</span>
                      <span style={{ fontSize:'11px', fontWeight:700, padding:'3px 10px', borderRadius:'999px', background:order.marca==='AREPA'?'#FEE2E2':'#DBEAFE', color:oc }}>
                        {order.marca === 'AREPA' ? '🫓 Arepa' : '🍔 Smash'}
                      </span>
                      <span style={{ fontSize:'11px', fontWeight:700, padding:'3px 10px', borderRadius:'999px', background:`${sc}18`, color:sc }}>
                  ...
                  <div style={{ padding:'14px 16px' }}>
                    ...
                    {(order as any).direccion_texto && (
                      <div style={{ fontSize:'12px', color:'#6B7280', marginBottom:'12px', display:'flex', gap:'6px' }}>
                        <span>📍</span><span>{(order as any).direccion_texto}</span>
                      </div>
                    )}

                    {/* Comprobante - siempre visible si existe */}
                    {comprobante && (
                      <a href={comprobante} target="_blank" rel="noreferrer"
                        style={{ display:'inline-flex', alignItems:'center', gap:'6px', fontSize:'12px', fontWeight:700, color:'#7C3AED', background:'#EDE9FE', padding:'8px 14px', borderRadius:'10px', textDecoration:'none', marginBottom:'12px' }}>
                    ...
                    <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
                      {estado === 'PENDIENTE' && (
                        <>
                          <button onClick={(e) => { ripple(e); updateStatus(order.id, 'PAGADO') }} className="rpl"
                            style={{ padding:'10px 18px', background:'#10B981', color:'white', border:'none', borderRadius:'999px', fontWeight:700, fontSize:'13px', cursor:'pointer', position:'relative' }}>
                            ✅ Aprobar
                          </button>
                          <button onClick={(e) => { ripple(e); updateStatus(order.id, 'CANCELADO') }} className="rpl"
                            style={{ padding:'10px 18px', background:'#EF4444', color:'white', border:'none', borderRadius:'999px', fontWeight:700, fontSize:'13px', cursor:'pointer', position:'relative' }}>
                            ❌ Rechazar
                          </button>
                        </>
                      )}
                      {estado === 'PAGADO' && (
                        <>
                          <button onClick={(e) => { ripple(e); updateStatus(order.id, 'EN_COCINA') }} className="rpl"
                            style={{ padding:'10px 18px', background:'#10B981', color:'white', border:'none', borderRadius:'999px', fontWeight:700, fontSize:'13px', cursor:'pointer', position:'relative' }}>
                            ✅ Aceptar → Cocina
                          </button>
                          <button onClick={(e) => { ripple(e); updateStatus(order.id, 'CANCELADO') }} className="rpl"
                            style={{ padding:'10px 18px', background:'#FEE2E2', color:'#DC2626', border:'none', borderRadius:'999px', fontWeight:700, fontSize:'13px', cursor:'pointer', position:'relative' }}>
                            ❌ Cancelar
                          </button>
                        </>
                      )}
                      {estado === 'EN_COCINA' && (
                        <button onClick={(e) => { ripple(e); updateStatus(order.id, 'LISTO') }} className="rpl"
                          style={{ padding:'10px 18px', background:'#3B82F6', color:'white', border:'none', borderRadius:'999px', fontWeight:700, fontSize:'13px', cursor:'pointer', position:'relative' }}>
                          ✓ Marcar Listo
                        </button>
                      )}
                      {estado === 'LISTO' && (
                        <button onClick={(e) => { ripple(e); updateStatus(order.id, 'EN_CAMINO') }} className="rpl"
                          style={{ padding:'10px 18px', background:'#8B5CF6', color:'white', border:'none', borderRadius:'999px', fontWeight:700, fontSize:'13px', cursor:'pointer', position:'relative' }}>
                          🛵 En Camino
                        </button>
                      )}
                      {estado === 'EN_CAMINO' && (
                        <button onClick={(e) => { ripple(e); updateStatus(order.id, 'ENTREGADO') }} className="rpl"
                          style={{ padding:'10px 18px', background:'#10B981', color:'white', border:'none', borderRadius:'999px', fontWeight:700, fontSize:'13px', cursor:'pointer', position:'relative' }}>
                          🎉 Entregado
                        </button>
                      )}
                      <button onClick={(e) => { ripple(e); printComanda(order) }} className="rpl"
                        style={{ padding:'10px 18px', background:oc, color:'white', border:'none', borderRadius:'999px', fontWeight:700, fontSize:'13px', cursor:'pointer', position:'relative' }}>
                        🖨️ Comanda
                      </button>
                    </div>
                  </div>
        ...
        {tab === 'resumen' && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
            {[
              { l:'Pedidos hoy', v:stats.pedidos, i:'📦' },
              { l:'Ingresos hoy', v:formatRD(stats.ingresos), i:'💰' },
              { l:'Clientes únicos', v:stats.nuevos, i:'👤' },
              { l:'Rating', v:'⭐ 4.8', i:'⭐' },
            ].map(s => (
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                  ...
                  <div>
                    <div style={{ display:'flex', alignItems:'center', gap:'6px' }}>
                      <span style={{ fontFamily:'var(--font-display)', fontWeight:800, fontSize:'15px' }}>#{(order as any).numero_pedido}</span>
