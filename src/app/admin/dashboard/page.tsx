...
export default function AdminDashboard() {
  ...
  function printComanda(order: Order) {
    ...
    const logoUrl = order.marca === 'AREPA' 
      ? window.location.origin + '/logos/logo-arepa.png'
      : window.location.origin + '/logos/logo-smash.png'
    ...
    const itemsHtml = items.map((i: any) => {
      const modifiers = (i.modifiers || i.selected_modifiers || [])
      const modHtml = modifiers.length > 0 
        ? modifiers.map((m: any) => `<div style="padding:2px 0 2px 12px;color:#555;font-size:13px;border-left:2px solid #ddd;margin-left:8px">+ ${m.nombre || m.name || m.option_name}${m.precio > 0 ? ' (+RD
...
+m.precio+')' : ''}</div>`).join('')
        : ''
      const notas = i.notas ? `<div style="padding:2px 0 2px 12px;color:#888;font-size:12px;font-style:italic;margin-left:8px">📝 ${i.notas}</div>` : ''
      return `
        <div style="padding:10px 0;border-bottom:1px dashed #e0e0e0">
          <div style="display:flex;justify-content:space-between;align-items:flex-start">
            <div style="font-size:15px;font-weight:700;color:#111">${i.cantidad}x ${i.product?.nombre || 'Producto'}</div>
            <div style="font-size:14px;font-weight:600;color:#333">RD${((i.product?.precio || 0) * i.cantidad).toLocaleString('es-DO')}</div>
          </div>
          ${modHtml}
          ${notas}
        </div>
