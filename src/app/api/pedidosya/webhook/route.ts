...
// Mapeo de eventos PedidosYa a estados de tu app
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!shippingId || !eventCode) {
      return NextResponse.json({ ok: true })
    }

    const nuevoEstado = EVENT_MAP[eventCode]
    if (!nuevoEstado) {
      return NextResponse.json({ ok: true, skipped: eventCode })
    }

    // Buscar la orden por pedidosya_shipping_id
    ...
    if (!Array.isArray(orders) || orders.length === 0) {
      console.log('Orden no encontrada para shippingId:', shippingId)
      return NextResponse.json({ ok: true, notFound: shippingId })
    }
    ...
    const updateUrl = `${SUPA_URL}/rest/v1/orders?id=eq.${order.id}`
    ...
    console.log(`Pedido #${order.numero_pedido} actualizado a ${nuevoEstado} por evento ${eventCode}`)
